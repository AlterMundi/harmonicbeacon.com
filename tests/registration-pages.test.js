...
test('registration form requires v4 terms and never bypasses its backend', () => {
  ...
  assert.match(page, /Entrada gratuita/);
  ...
  assert.match(page, /<b data-option-name>ENGLISH<\/b>[\s\S]*?Sunday · Costa Rica time[\s\S]*?<b>16:00<\/b>[\s\S]*?FREE/);
  ...
  assert.match(page, /applyFreeCopy/);
  assert.match(page, /INSCRIPCIÓN PRIVADA GRATUITA/);
  assert.match(page, /FREE PRIVATE REGISTRATION/);
  assert.match(page, /No card is required/);
